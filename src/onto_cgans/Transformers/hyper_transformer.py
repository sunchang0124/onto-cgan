...
from onto_cgans.Transformers.base import BaseTransformer, BaseMultiColumnTransformer
from onto_cgans.Transformers.transformers import FloatFormatter, UniformEncoder, UnixTimestampEncoder
from onto_cgans.utils import WarnDict
...
class HyperTransformer:
    ...
    def update_sdtypes(self, column_name_to_sdtype):
        ...
        if len(self.field_sdtypes) == 0:
            raise ValueError(self._DETECT_CONFIG_MESSAGE)
    ...
    def _validate_fitted(self):
        if not self._fitted or self._modified_config:
            raise ValueError(self._NOT_FIT_MESSAGE)
