...
from onto_cgans.onto_base import Onto_BaseTabularModel
...
class Onto_DPCGANModel(Onto_BaseTabularModel):
    ...
    def _sample_rows(self, num_rows, unseen_rds=[], sort=True, conditions=None,
        ...
        """Sample rows and re-attach IRI column after Table.reverse_transform strips it."""
        sampled, num_valid = super()._sample_rows(
            num_rows, unseen_rds, sort, conditions, transformed_conditions,
            float_rtol, previous_rows
        )
        model = self._model
        if not (hasattr(model, '_last_sampled_rds') and model._last_sampled_rds):
            return sampled, num_valid

        rd_col = model._rd_column_name
        if rd_col in sampled.columns:
            # Retry path: previous_rows already have IRI; fill NaN slots for new rows
            null_mask = sampled[rd_col].isna()
            new_count = int(null_mask.sum())
            if new_count > 0:
                sampled.loc[null_mask, rd_col] = model._last_sampled_rds[:new_count]
        else:
            sampled.insert(0, rd_col, model._last_sampled_rds[:len(sampled)])
